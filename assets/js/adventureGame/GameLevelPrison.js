...
class GameLevelPrison {
  constructor(path) {
    ...
    const getRandomPosition = (scaleFactor) => {
      return {
        x: Math.floor(Math.random() * (width - 32 * scaleFactor)) + 200,
        y: Math.floor(Math.random() * (height - 32 * scaleFactor)) + 200
      };
    };
    ...
    const sprite_data_knight = {
        ...
        ANIMATION_RATE: 50,
        pixels: { height: 64, width: 64 },
        INIT_POSITION: { x: (width / 1.6), y: (height / 4) - 100 }, // Shifted right by increasing x value
        orientation: { rows: 1, columns: 1 },
        down: { row: 0, start: 0, columns: 1 },
        hitbox: { widthPercentage: 0.1, heightPercentage: 0.2 },
        level_data: levelData
